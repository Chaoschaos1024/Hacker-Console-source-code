...
#define x_times 41
#define y_times 68
bool is_there_a_touch_display_on_rp2040 = 0;
...
unsigned int loop_count = 0;  // 最高200
unsigned int loop_count_max = 200;
const unsigned int count_plus = 100;
...
void setup() {
...
#if debug
  Serial1.println("keyboard iic report :");
  for (int i = 0; i < keyboard_iic_length; i++) {
    Serial1.print(i);
    Serial1.print(" : ");
  ...
  is_there_a_touch_display_on_rp2040 = !Wire1.endTransmission();
#if debug
  Serial1.print("是否有触摸板：");
  if (is_there_a_touch_display_on_rp2040) {
    Serial1.println("有");
  } else {
    Serial1.println("没有");
  }
  Serial1.println(is_there_a_touch_display_on_rp2040);
#endif
  if (is_there_a_touch_display_on_rp2040) {
...
bool keyboard() {
  ...
  if (iic_report[keyboard_iic_length - 1] == keyboard_iic_crc) {
    ...
    if (mouse_dx < 128 + dead_zone) {
      if (mouse_dx > 128 - dead_zone) {
        mouse_dx = 128;
      }
    }
    int mouse_dy = iic_report[1];
    if (mouse_dy < 128 + dead_zone) {
      if (mouse_dy > 128 - dead_zone) {
        mouse_dy = 128;
      }
    }
    ...
    if (iic_report[2] == 66) {
      if (keyboard_matrix[6][0] != keyboard_matrix_last_time[6][0]) {
        if (keyboard_matrix[6][0] == 1) {
          backlight_duty_cycle_new = backlight_duty_cycle_new + led_step;
...
void hdmi_update() {
  bool screen_now = digitalread(hdmi_active);
  if (screen_enabled != screen_now) {
    if (screen_enabled) {
      ...
      screen_enabled = 1;
      need_to_update_backlight = 1;
    }
  } else {
  }
  //  digitalWrite(screen_enable, digitalread(hdmi_active));
...
void adc() {
  ...
  //battery_percent = map(battery_voltage, battery_voltage_min, battery_voltage_max, 0, 100);
  if (battery >= 100) {
    battery_percent = 100;
  } else if (battery_percent <= 0) {
    battery_percent = 0;
